...
public class LuaVM
{
    ...
    /// </summary>
    public LuaVM()
        : this(VMSettings.AttachAll, new[] {""}, CoreModules.Preset_SoftSandbox)
    {
    }
    ...
    public Table GetGlobalTable(string key)
    {
        ...
        if (tableDyn != null)
        {
            ...
            {
                Logger.Log(Channel.Lua, Priority.FatalError, "Lua global {0} is not type table, has type {1}", key,
                    tableDyn.Type.ToString());
            }
        }
        return result;
    }
    ...
    public Table GetGlobalTable(params object[] keys)
    {
        ...
        if (tableDyn != null)
        {
            ...
            {
                Logger.Log(Channel.Lua, Priority.FatalError, "Lua global {0} is not type table, has type {1}", keys,
                    tableDyn.Type.ToString());
            }
        }
        return result;
    ...
    public DynValue Call(DynValue luaFunc, int num)
    {
        return Call(luaFunc, DynValue.FromObject(m_LuaScript, num));
    }

    public DynValue Call(DynValue luaFunc, float num)
    {
        return Call(luaFunc, DynValue.FromObject(m_LuaScript, num));
    }
    ...
    public DynValue Call(string functionName, params object[] args)
    {
        ...
        if (!string.IsNullOrEmpty(functionName))
        {
            ...
            if (func.Type == DataType.Function)
            {
                ...
                {
                    Logger.Log(Channel.Lua, Priority.FatalError, "Lua error calling function {0}: {1}", functionName,
                        ex.DecoratedMessage);
                }
            }
            else
